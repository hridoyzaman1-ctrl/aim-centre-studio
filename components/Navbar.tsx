    ...
    >
      <div className="max-w-screen-2xl mx-auto px-5 md:px-12 flex items-center justify-between">
        <div
          ...
          aria-label="AIM Centre 360 Home"
        >
        ...
        <div className="hidden md:flex items-center gap-10">
          {['Home', 'Curriculum', 'Special Needs', 'Mental Wellbeing', 'Courses'].map((item) => (
            <a
              key={item}
              href={item === 'Home' ? '/' : item === 'Mental Wellbeing' ? '#counseling' : item === 'Courses' ? '#courses' : item === 'Curriculum' ? '#technology' : `#${item.toLowerCase().replace(' ', '-')}`}
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-6 mt-10">
              {['Home', 'Curriculum', 'Special Needs', 'Mental Wellbeing', 'Courses'].map((item, idx) => (
                <motion.a
